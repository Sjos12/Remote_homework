<template>
    <div>
        <div
            ...
            v-on:keyup.left="movePrev()"
            v-on:keyup.right="moveNext()"
        >
            <div class="carousel" id="carousel">
                <div
                    v-for="slide in slidesArray"
                    :key="slide.slide"
                    class="w-100 carousel__slide"
                    :class="slide.class"
                >
                    <div class="imagedrop flex flex-column align-items-center">
                        <input
                        ...
                        <div class="imagedrop__markup" v-if="!isImageUrlSet()">
                            <i
                                class="fa fa-camera fa-5x imgdrop__markup"
                                alt="Camera Icon"
                            ></i>
                    ...
                    <button
                        type="button"
                        v-on:click="movePrev()"
                        class="carousel__button--previous mx-4"
                        :class="prevBtnClass"
                    ...
                    <button
                        type="button"
                        v-on:click="moveNext()"
                        class="carousel__button--next mx-4"
                        :class="nextBtnClass"
                ...
                <div class="flex justify-center align-items-center w-100">
                    <div
                        v-for="slide in slidesArray"
                        :key="slide.slide"
                        class="mr-1"
                    >
                        <i
                            class="fa fa-circle"
                            v-bind:class="{
                                'blue fa-lg': isActiveSlide(slide.slide),
                                'white fa-sm': !isActiveSlide(slide.slide),
                            }"
                        ></i>
                    </div>
                    <button
                        type="button"
                        v-on:click="addSlide()"
                        class="btn shadow-none enabled z-high"
                    >
            ...
            <small class="form-text text-muted">
                Click the camera field to add images, use the left right keys to
                navigate and add more images to your question.
            </small>
        </div>
    </div>
</template>
...
export default {
    data() {
        return {
            slidesArray: [{ slide: 0, class: " active", url: "" }],
            activeSlide: 0,
            prevBtnClass: "",
            nextBtnClass: "",
            maxSlides: 10,
    ...
    methods: {
        ...
        dragndrop: function (event) {
            console.log("change");
            this.slidesArray[this.activeSlide].url = URL.createObjectURL(
                event.target.files[0]
            );
        },
        drag: function () {
            document.getElementById("uploadFile").parentNode.className =
                "imagedrop--dragging imagedrop";
        },
        drop: function () {
            console.log("drop");
            document.getElementById("uploadFile").parentNode.className =
